...
int *read_tokenized_data(char *filename, size_t *read)
{
    ...
    while(one == 1){
        ...
        if(count > size){
            size = size*2;
            d = (int *)realloc((void *)d, size*sizeof(int));
        }
        d[count-1] = n;
        one = fscanf(fp, "%d", &n);
    }
    fclose(fp);
    d = (int *)realloc((void *)d, count*sizeof(int));
    *read = count;
    return d;
}
...
char **read_tokens(char *filename, size_t *read)
{
    ...
    while((line=fgetl(fp)) != 0){
        ...
        if(count > size){
            size = size*2;
            d = (char **)realloc((void *)d, size*sizeof(char *));
        }
        if(0==strcmp(line, "<NEWLINE>")) line = (char *)"\n";
        d[count-1] = line;
    }
    fclose(fp);
    d = (char **)realloc((void *)d, count*sizeof(char *));
    *read = count;
    return d;
}
