...
tree *read_tree(char *filename)
{
    ...
    while((line=fgetl(fp)) != 0){
        ...
        t.parent = (int *)realloc((void *)t.parent, (n+1)*sizeof(int));
        t.parent[n] = parent;

        t.child = (int *)realloc((void *)t.child, (n+1)*sizeof(int));
        t.child[n] = -1;

        t.name = (char **)realloc((void *)t.name, (n+1)*sizeof(char *));
        t.name[n] = id;
        if(parent != last_parent){
            ++groups;
            t.group_offset = (int *)realloc((void *)t.group_offset, groups * sizeof(int));
            t.group_offset[groups - 1] = n - group_size;
            t.group_size = (int *)realloc((void *)t.group_size, groups * sizeof(int));
            t.group_size[groups - 1] = group_size;
            group_size = 0;
            last_parent = parent;
        }
        t.group = (int *)realloc((void *)t.group, (n+1)*sizeof(int));
        t.group[n] = groups;
        if (parent >= 0) {
